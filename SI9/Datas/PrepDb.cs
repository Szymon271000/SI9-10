...
namespace SI9.Datas
{
    public static class PrepDb
    {
        public static void PrepPopulation(IApplicationBuilder app)
        {
            ...
            {
                SeedData(serviceScope.ServiceProvider.GetService<DataStructuresAndAlghoritmsContext>());
            }
        }

        private static void SeedData(DataStructuresAndAlghoritmsContext context)
        {
            if (!context.dataStructures.Any())
            {
                Console.WriteLine("--> Seeding data");
                context.dataStructures.AddRange(
                    new DataStructure
                    {
                        Id = 1,
                        Name = "Stack",
